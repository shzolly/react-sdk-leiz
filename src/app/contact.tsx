...
const Contact = () => {
  ...
  function createInquiryCase() {
    let token = '';
    (async () => {
      const responseToken = await fetch('https://njcts-njcc-dt1.pegacloud.net/prweb/PRRestService/oauth2/v1/token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          Authorization: `custom-bearer`
        },
        body: 'client_id=91139000849630578756&grant_type=custom-bearer&enable_psyncId=true'
      });
      ...
      const responseCase = await fetch('https://njcts-njcc-dt1.pegacloud.net/prweb/api/application/v2/cases?viewType=none&pageName=', {
        method: 'POST',
        headers: {
